...
== Subscription not deploying

WIP, I've broken something causing the sub not to create a Helmrelease anymore.

[source,]
$ oc get subscriptions/stackrox-secured-cluster-services -n stackrox
NAME                                SUBSCRIPTIONSTATE   APPSTATUSREFERENCE   AGE    UPDATED   LOCAL PLACEMENT   TIME WINDOW
stackrox-secured-cluster-services                                            107m
$ oc get helmreleases -n stackrox
No resources found in stackrox namespace.
